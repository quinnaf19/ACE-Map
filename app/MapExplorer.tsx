...
export default function MapExplorer() {
  ...
  const loadData = () => {
    ...
    setError("");
    Promise.all([
      fetch("/data/stops.json").then((r) => {
        if (!r.ok) throw new Error("Stops data request failed");
        return r.json();
      }),
      fetch("/data/manhattan-ntas.geojson").then((r) => {
      ...
      .catch(() => {
        setLoading(false);
        setError("Map data could not be loaded. Check your connection and try again.");
      });
  };
  ...
  const routes = useMemo(
    () => [
      "All routes",
      ...Array.from(new Set(rows.map((r) => r.route))).sort((a, b) =>
  ...
  const ntas = useMemo(
    () => ["All neighborhoods", ...Array.from(new Set(rows.map((r) => r.nta))).sort()],
    [rows]
  );
  ...
  const points = useMemo(() => {
    ...
    rows
      .filter(
        (r) =>
          (route === "All routes" || r.route === route) &&
          (nta === "All neighborhoods" || r.nta === nta) &&
  ...
  useEffect(() => {
    if (!mapRef.current || !geo || !leaflet) return;
    if (ntaLayer.current) ntaLayer.current.remove();
    ntaLayer.current = leaflet
      .geoJSON(geo, {
        style: (f: any) => ({
          color: f?.properties?.ntaname === nta ? "#111827" : "#64748b",
          weight: f?.properties?.ntaname === nta ? 2.5 : 0.7,
          fillColor: "#f8fafc",
          fillOpacity: f?.properties?.ntaname === nta ? 0.18 : 0.045,
        }),
        onEachFeature: (f, l) => {
          l.bindTooltip(f.properties.ntaname, { sticky: true, className: "nta-tip" });
          l.on("click", () => setNta(f.properties.ntaname));
        },
      })
  ...
  const focus = (p: MapPoint) => {
    setSelected(p);
    mapRef.current?.flyTo([p.latitude, p.longitude], 16, { duration: 0.7 });
  };
  ...
  const reset = () => {
    ...
    setMin(1);
    setSelected(null);
    mapRef.current?.flyTo([40.783, -73.971], 12);
  };

  // Render a clean loading shell when executing on server side
  if (!isMounted) {
    return (
      ...
      <section className="workspace">
        <aside className="sidebar">
          <div className="side-head">
            <div>
              <span className="step">FILTER THE MAP</span>
          ...
          <div className="filter-grid">
            ...
            <label>
              Neighborhood
              <select value={nta} onChange={(e) => setNta(e.target.value)}>
                {ntas.map((x) => (
                  <option key={x}>{x}</option>
                ))}
              </select>
          ...
          <label>
            ...
            <div className="search">
              ...
              <input
                ...
                placeholder="Search Broadway, W 125 ST…"
              />
          ...
          <div className="kpis">
            ...
            <div>
              ...
              <strong>{fmt.format(points.length)}</strong>
            </div>
            <div>
              <span>Routes visible</span>
              <strong>{fmt.format(new Set(points.flatMap((p) => p.routes)).size)}</strong>
            </div>
          </div>
